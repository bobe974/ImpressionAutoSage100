...
;Nom des fenetres possible si sage est déja connecté a une base
Local $tabFenetres[3]
$tabFenetres[0] = "REDISMA - Sage 100 Gestion commerciale Premium"
$tabFenetres[1] = "STOCKSERVICE - Sage 100 Gestion commerciale Premium"
$tabFenetres[2] = "DISBEP - Sage 100 Gestion commerciale Premium"



Global $LockFile = @ScriptDir & "\AutoItLockFile.txt"  ;répertoire actuelle du script
Global $LogFile = @ScriptDir &  "\autoItLog.txt"       ;Log
$actualWindowName = "Sage 100 Gestion commerciale Premium" 
...
WinActivate ($actualWindowName)

; Vérifier si la fenêtre Sage vierge existe
If WinExists($actualWindowName) Then
    WriteToLog($LogFile, "une fenetre sage 100 est déja ouverte: Non connecté à une bdd")
    ; Connexion à la base de données
    OuvrirFichierGcm()
	$actualWindowName = "Sage 100 Gestion commerciale Premium"
Else
    ; Vérifier si la fenêtre Sage est active avec la base de données actuelle
    If WinExists($dbName & " - Sage 100 Gestion commerciale Premium") Then
		WinActivate ($dbName & " - Sage 100 Gestion commerciale Premium")
		WriteToLog($LogFile, "une fenetre sage 100 est déja ouverte: Deja connecté à la base: "& $dbName & " - Sage 100 Gestion commerciale Premium")
        ; On ne fait rien, la connexion a déjà été établie
		$actualWindowName = $dbName & " - Sage 100 Gestion commerciale Premium"
    Else
		$Wname = GetWindowName($tabFenetres)
        ;cas ou sage est connecté à une autre base	
        If $Wname Then
			WriteToLog($LogFile, "une fenetre sage 100 est déja ouverte: Connecté à la mauvaise bdd, deconnexion...")
			; Déconnexion de la base de données actuelle
			Sleep(2000)
			;ControlSend()
            Send("^w")                ;TODOOOOOOOOOOOOOOOOOOO CONTROL SEND
			Sleep(2000)
			$actualWindowName = $Wname
            ; Reconnexion à la nouvelle base de données
            OuvrirFichierGcm()
        Else  ;cas ou sage100 n'est pas lancé
...
; variable qui contient le nom de la bonne fenetre
	
;+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

;***********************************************************************************************************************************************************
WriteToLog($LogFile, "Nom actuel de la fenetre: " & $actualWindowName)

;**************************************************Gestion du message bloquant sur la fermeture fiscal******************************************************
WriteToLog($LogFile,"gestion du message de fermeture fiscal")
Sleep(2000)
WinActivate ($actualWindowName)              
If WinWaitActive($actualWindowName,"",10) Then  
...
WinActivate($dbName&" - Sage 100 Gestion commerciale Premium")
If WinWaitActive($dbName&" - Sage 100 Gestion commerciale Premium","",15) Then ; Attend jusqu'à 15 secondes pour que la fenêtre soit active
...
; Cliquer sur le bouton "Actions"
	...
	Sleep(4000) ;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	EtapeDeVerification("Impression Liste de documents", $dbName&" - Sage 100 Gestion commerciale Premium")  ;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	Else
		...
		if WinWaitActive("Impression", "", 20) then
			EtapeDeVerification("Impression", $dbName&" - Sage 100 Gestion commerciale Premium")
...
Func GetWindowName($tab)
	$windowName = Null
	; Boucle For Next pour parcourir le tableau
	For $i = 0 To UBound($tab) - 1
		if WinExists($tab[$i]) Then
			$windowName = $tab[$i]
			WriteToLog($LogFile, " GetWindowsName -> Nom de fenetre trouvé:" & $windowName)
...
Func EtapeDeVerification($fenetreActuel, $fenetrePrecdente)
    ...
    If WinExists($fenetreActuel) Then
        ...
        EndIf    
    Else 
		if WinExists($fenetrePrecdente) Then ; ferme la fenetre précédente et déconnection de la bdd
			WriteToLog($LogFile,"la fenetre précédente existe, fermeture de la fenetre et déconnexion..")
			;WinClose($fenetrePrecdente)
			Send("^w")
			DeleteLockFile($LockFile)
			Exit(0) ; Arrête le script

		Else  ; fermeture du programme car on ne peux pas faire la commande pour se déconencter
